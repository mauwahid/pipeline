package mandiri.pipeline.act.solution;
...
public class CycleSolutionAct extends GenericForwardComposer<Component>  implements Act<CycleSolution>{
	...
	private LogSolutionDao logSolutionDao;
	private UserSalesDao userSalesDao;
	private UserSolution user;
	...
	public void loadData() {
		// TODO Auto-generated method stub
			user = (UserSolution)session.getAttribute("pengguna");
			
			System.out.println("User Id "+user.getId());
		
			cycleSolutionDao = HibernateUtil.getCycleSolutionDao();
			List<CycleSolution> list = cycleSolutionDao.getAsUserSolution(user);
	...
	public void showForm(CycleSolution entity) {
		// TODO Auto-generated method stub
		if(entity!=null)
			this.cycleSolution = entity;
		else
			this.cycleSolution = new CycleSolution();
		...
		cbItem.setLabel(Common.SALES_NO_CYCLE_1 +". "+Common.SOLUTION_CYCLE_1);
		cbItem.setValue(Common.SALES_NO_CYCLE_1);
		cbItem.setParent(cbSalesCycle);
		cbItem = new Comboitem();
		cbItem.setLabel(Common.SALES_NO_CYCLE_2 +". "+Common.SOLUTION_CYCLE_2);
		cbItem.setValue(Common.SALES_NO_CYCLE_2);
		cbItem.setParent(cbSalesCycle);
		cbItem = new Comboitem();
		cbItem.setLabel(Common.SALES_NO_CYCLE_3 +". "+Common.SOLUTION_CYCLE_3);
		cbItem.setValue(Common.SALES_NO_CYCLE_3);
		cbItem.setParent(cbSalesCycle);
		cbItem = new Comboitem();
		cbItem.setLabel(Common.SALES_NO_CYCLE_4 +". "+Common.SOLUTION_CYCLE_4);
		...
		btnSave.addEventListener("onClick", new EventListener<Event>() {
			...
			public void onEvent(Event event) throws Exception {
				// TODO Auto-generated method stub
				cycleSolutionDao = HibernateUtil.getCycleSolutionDao();
				LogSolution logSolution = new LogSolution();
				CycleSolution cs;
				if(cycleSolution==null){
					cs = new CycleSolution();
				...
				cs.setDepartment(tbDepartment.getText());
				...
				if(cbSalesCycle.getSelectedItem()!=null)
					cs.setSolutionCycle((int)cbSalesCycle.getSelectedItem().getValue());
				
				
				logSolution.setComments(tbComments.getText());
				logSolution.setCloseImplDate(dBoxCloseImplDate.getValue().toLocaleString());
				logSolution.setEstAddAvg(tbEstAddAvg.getText());
				logSolution.setEstAddFBI(tbEstAddFBI.getText());
				logSolution.setExpCloseDate(dBoxExpCloseDate.getValue().toLocaleString());
				logSolution.setWonDate(dBoxWonDate.getValue().toLocaleString());
				logSolution.setUserSales(user.getUsername());
				logSolution.setTglperubahanAkhir(new Date().toLocaleString());
				
				
				if(cbGroupCustomer.getSelectedItem()!=null)
					{
						GroupCustomer gc = (GroupCustomer)cbGroupCustomer.getSelectedItem().getValue();
						logSolution.setGroupCustomer(gc.getGroupName());
					}
				
				if(cbCustomer.getSelectedItem()!=null){
					Customer customer = (Customer)cbCustomer.getSelectedItem().getValue();
					logSolution.setCustomer(customer.getName());
				}
			
				if(cbAnchor.getSelectedItem()!=null){
					Anchor anc = (Anchor)cbAnchor.getSelectedItem().getValue();
					logSolution.setAnchor(anc.getNama());
				}
				
				logSolution.setCategory(tbCategory.getValue());
				
				if(cbBUC.getSelectedItem()!=null){
					BUC buc = (BUC)cbBUC.getSelectedItem().getValue();
					logSolution.setBuc(buc.getNama());
				}
					
				if(cbProduct.getSelectedItem()!=null){
					Product prod = (Product)cbProduct.getSelectedItem().getValue();
					logSolution.setProduct(prod.getNama());
					
				}
					
				logSolution.setNextAction(tbNextAction.getValue());
				logSolution.setOpenDate(dBoxOpenDate.getValue().toLocaleString());
				logSolution.setRealDate(dBoxDealDate.getValue().toLocaleString());
				logSolution.setRelationshipManager(tbRelationshipManager.getValue());
				
				if(cbSalesCycle.getSelectedItem()!=null){
					int value = (int)cbSalesCycle.getSelectedItem().getValue();
					if(value==1)
						logSolution.setSolutionCycle(Common.SALES_NO_CYCLE_1 + ". "+Common.SOLUTION_CYCLE_1);
					else if(value==2)
						logSolution.setSolutionCycle(Common.SALES_NO_CYCLE_2 + ". "+Common.SOLUTION_CYCLE_2);
					else if(value==3)
						logSolution.setSolutionCycle(Common.SALES_NO_CYCLE_3 + ". "+Common.SOLUTION_CYCLE_3);
					else if(value==4)
						logSolution.setSolutionCycle(Common.SALES_NO_CYCLE_4 + ". "+Common.SOLUTION_CYCLE_4);
					}
			
				
				if(cycleSolution==null)
					cycleSolutionDao.insert(cs);
				else
					cycleSolutionDao.update(cs);
	...
	private void deleteData(CycleSolution dataTemp) {
		// TODO Auto-generated method stub
		cycleSolutionDao = HibernateUtil.getCycleSolutionDao();
		cycleSolutionDao.delete(dataTemp);
		loadData();
		
		
	}
	
public void detail(CycleSolution entity) {
	...
	//	grid.setParent(window);
		...
		row.appendChild(new Label("Solution Cycle"));
		if(entity!=null){
			if(entity.getSolutionCycle()!=0){
				if(entity.getSolutionCycle()==1)
					row.appendChild(new Label(Common.SALES_NO_CYCLE_1 + ". "+Common.SOLUTION_CYCLE_1));
				else if(entity.getSolutionCycle()==2)
					row.appendChild(new Label(Common.SALES_NO_CYCLE_2 + ". "+Common.SOLUTION_CYCLE_2));
				else if(entity.getSolutionCycle()==3)
					row.appendChild(new Label(Common.SALES_NO_CYCLE_3 + ". "+Common.SOLUTION_CYCLE_3));
				else if(entity.getSolutionCycle()==4)
					row.appendChild(new Label(Common.SALES_NO_CYCLE_4 + ". "+Common.SOLUTION_CYCLE_4));
						else
					row.appendChild(new Label(""));
			}
		}
		...
		row.setParent(rows);
		row.appendChild(new Label("Open Date"));
		row.appendChild(new Label(cycleSolution!=null?cycleSolution.getOpenDate().toLocaleString():""));
		row.setParent(rows);
		
		row = new Row();
		row.setParent(rows);
		row.appendChild(new Label("Expected Close Date"));
		row.appendChild(new Label(cycleSolution!=null?cycleSolution.getExpCloseDate().toLocaleString():""));
		row.setParent(rows);
		
		row = new Row();
		row.setParent(rows);
		row.appendChild(new Label("Won Date"));
		row.appendChild(new Label(cycleSolution!=null?cycleSolution.getWonDate().toLocaleString():""));
		row.setParent(rows);
		
		row = new Row();
		row.setParent(rows);
		row.appendChild(new Label("Close Implementation Date"));
		row.appendChild(new Label(cycleSolution!=null?cycleSolution.getCloseImplDate().toLocaleString():""));
		row.setParent(rows);
		
		row = new Row();
		row.setParent(rows);
		row.appendChild(new Label("Deal Date"));
		row.appendChild(new Label(cycleSolution!=null?cycleSolution.getRealDate().toLocaleString():""));
		row.setParent(rows);
		
		row = new Row();
		row.setParent(rows);
		row.appendChild(new Label("Comments"));
		row.appendChild(new Label(cycleSolution!=null?cycleSolution.getComments():""));
		row.setParent(rows);
		
		row = new Row();
		row.setParent(rows);
		row.appendChild(new Label("Next Action"));
		row.appendChild(new Label(cycleSolution!=null?cycleSolution.getNextAction():""));
		row.setParent(rows);
	...
	void showLog(CycleSolution entity) {
	...
	//	listLogStatus.setParent(window);
		...
		logSolutionDao = HibernateUtil.getLogSolutionDao();
		List<LogSolution> logSolutiones = logSolutionDao.getLogSolution(entity);
		listModel = new ListModelList<LogSolution>(logSolutiones);
		listLogStatus.setModel(listModel);
		
		listLogStatus.setItemRenderer(new LogSolutionRenderer());
	...
	class LogSolutionRenderer implements ListitemRenderer<LogSolution>{

	
	@Override
	public void render(Listitem item, LogSolution entity, int index)
			throws Exception {
		// TODO Auto-generated method stub
		final LogSolution dataTemp = entity;
		index = index + 1;
