...
public class CustomerAct extends GenericForwardComposer<Component> implements Act<Customer>{
	...
	Listbox list_customer;
	
	private ListModelList<Customer> listModel;
	...
	public void loadData() {
		...
		//	listModel = new SimpleListModel<DistributionFinance>(list);
			...
			list_customer.setModel(listModel);
			list_customer.setItemRenderer(new CustomerRenderer());
			list_customer.renderAll();
			//setCheckBox();
			
			
	}
	
	class CustomerRenderer implements ListitemRenderer<Customer>{

		
		@Override
		public void render(Listitem item, Customer entity, int index)
				throws Exception {
			// TODO Auto-generated method stub
			final Customer dataTemp = entity;
			index = index + 1;
				
		
			new Listcell(dataTemp.getId()+"").setParent(item);
			new Listcell(dataTemp.getName()).setParent(item);
	...
	@Override
	public void showForm(Customer entity) {
		// TODO Auto-generated method stub
		if(entity!=null)
			customer = entity;
		...
		tbNama = new Textbox(customer!=null?customer.getName():"");
		...
		tbDeskripsi = new Textbox(customer!=null?customer.getAlamat():"");
		...
		btnSave.addEventListener("onClick", new EventListener<Event>() {
			...
			public void onEvent(Event event) throws Exception {
				...
				cus.setName(tbNama.getText());
				cus.setAlamat(tbDeskripsi.getValue());
	...
	private void deleteData(Customer dataTemp) {
		// TODO Auto-generated method stub
		customerDao = HibernateUtil.getCustomerDao();
		customerDao.delete(dataTemp);
	}
